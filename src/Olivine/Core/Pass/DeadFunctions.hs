-- | Removing functions no live path reaches.
--
...
-- whether a /function/ is dead is settled by looking at everything else, since
-- the one place its name cannot appear is the place a single-function pass
-- would look.
--
-- Two things make this more than removing functions nothing calls.
--
...
-- are dead.  So the live set is grown from roots — the functions the program
-- can be entered at, and the functions named by everything that is kept
-- regardless — and a function is dead when that growth never arrives at it.
--
...
module Olivine.Core.Pass.DeadFunctions
...
import Olivine.Syntax.Global (globalInitializer)
...
-- | Drop every function the live set does not contain.
--
-- Declarations go the same way.  A declaration is a function whose body is
-- somewhere else, and one nothing names emits nothing and says nothing; the
-- reachability that finds the definitions to remove has already worked out
-- which declarations are left over, and leaving them behind would mean this
-- pass could delete the only caller of @\@printf@ and still write out its
-- @declare@.
...
eliminateDeadFunctions program =
  ...
  where
    ...
    reached entry = case entry of
      EFunction f
        | removableWhenUnreached (functionSignature f) ->
...
-- | Whether a function can go when nothing live reaches it.
--
-- Its linkage is the whole of the question, and the answer is what linkage
-- means: whether the symbol can be reached from outside this module without
-- passing through anything inside it.
--
...
--
-- A function with no linkage written is external, and this pass can do
-- nothing with it — which, while a program is one module, is most of them.
-- When a whole program really is several modules linked together, external
-- linkage stops being the end of the question and only the entry point and
-- what the program is linked against are roots.  That is a fact about how
-- much of the program has been read, not about this function, so it will
-- change here and nowhere else.
removableWhenUnreached :: Signature -> Bool
removableWhenUnreached signature = case signatureLinkage signature of
...
-- distinguishes symbols by it, so nothing here may either.
reachableIn :: Program -> Set Text
reachableIn program = grow Set.empty (concatMap roots (programEntries program))
  where
    -- What each function that could be removed would carry with it if it
    -- turned out to be live.  The rest are roots and are walked as such.
    ...
    -- A function that cannot be removed is a way into the program, so it is
    -- live and so is everything it names.  Everything else kept regardless —
    -- globals, metadata, definitions the lowering could not take, text not
    -- yet read — contributes what it names and not itself.
    roots (EFunction f)
      | removableWhenUnreached (functionSignature f) = []
      | otherwise =
          nameText (signatureName (functionSignature f)) : referencesIn f
    roots (ERetained entry) = referencesInEntry entry
...
--
-- Metadata attachments are not read here.  An attachment refers to a node,
-- every node is an entry of its own and kept, and what the node names has
-- already been counted at the node.
referencesInEntry :: Syntax.Entry -> [Text]
referencesInEntry entry = case entry of
  Syntax.EGlobal g -> names (foldMap globalsIn (globalInitializer g))
...
-- a function.  A name in text the optimizer cannot read is a name it has to
