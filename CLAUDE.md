...
Where one construct is a subset of another — constants among operands, terminators among instructions — the form that has to read LLVM back uses a single type together with a predicate, not two parallel types. The core, which nothing but Olivine constructs, may instead make the invariant structural: a predicate is right where invalid input must survive to be reported, and structure is right where it cannot arise. LLVM expresses these relations as subtyping, which costs it nothing; Haskell has no subtyping, so encoding them costs a conversion at every boundary and a second copy of every function over the form. Validity belongs in a verifier pass rather than in the shape of the data, and unnecessary differences between the LLVM-facing syntax and the optimizer's own representation should be minimized, since every one of them is a translation somebody has to keep correct.
