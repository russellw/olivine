...
BOOST_AUTO_TEST_CASE(ValidFunctionWithAlloca) {
    ...
    vector<Inst> body = {
        block(Ref("entry")),
        alloca(createPtrVar("ptr"), intTy(32), intConst(intTy(32), 1)),
        ret()
    };
...
BOOST_AUTO_TEST_CASE(InvalidReturnType) {
    ...
    vector<Inst> body = {
        block(Ref("entry")),
        ret(intConst(intTy(32), 0))  // Returning int from void function
    };
...
BOOST_AUTO_TEST_CASE(InvalidBranchCondition) {
    ...
    vector<Inst> body = {
        block(Ref("entry")),
        br(params[0], Ref("then"), Ref("else")),  // Using int as condition
        block(Ref("then")),
        ret(intConst(intTy(32), 1)),
        block(Ref("else")),
        ret(intConst(intTy(32), 0))
...
BOOST_AUTO_TEST_CASE(InvalidPhiInstruction) {
    ...
    vector<Inst> body = {
        block(Ref("entry")),
        Inst(Phi, {result, intConst(intTy(32), 1), label(Ref("l1")),
                          intConst(intTy(32), 2), label(Ref("l2"))}),
        ret(result)
    };
...
BOOST_AUTO_TEST_CASE(InconsistentVariableTypes) {
    ...
    vector<Inst> body = {
        ...
        assign(params[0], intConst(intTy(64), 42)),
        ret(params[0])
    };
...
BOOST_AUTO_TEST_CASE(InvalidStore) {
    ...
    vector<Inst> body = {
        block(Ref("entry")),
        store(params[1], params[0]),  // First param should be a pointer
        ret()
    };
...
BOOST_AUTO_TEST_CASE(ValidComplexFunction) {
    ...
    vector<Inst> body = {
        ...
        br(Term(Eq, boolTy(), temp, intConst(intTy(32), 0)), 
           Ref("zero"), Ref("nonzero")),
...
BOOST_AUTO_TEST_CASE(MissingReturn) {
    ...
    vector<Inst> body = {
        block(Ref("entry")),
        assign(createIntVar("y"), params[0])
