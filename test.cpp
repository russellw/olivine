...
BOOST_AUTO_TEST_CASE(SingleElementVectorTest) {
    vector<int> v{42};
    size_t expected = 0;
    hash_combine(expected, 42);
    BOOST_CHECK_EQUAL(hashVector(v), expected);
}
...
BOOST_AUTO_TEST_CASE(StringVectorTest) {
    ...
    size_t expected = 0;
    hash_combine(expected, string("hello"));
    hash_combine(expected, string("world"));
    BOOST_CHECK_EQUAL(hashVector(v), expected);
}
