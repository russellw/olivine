...
public:
    Item(const std::string& name, const std::string& description, int value, 
         bool isWeapon = false, int damage = 0,
         bool isArmor = false, int defense = 0,
         bool isConsumable = false, int healthRestore = 0)
        : name(name), description(description), value(value), 
          isWeapon(isWeapon), damage(damage),
          isArmor(isArmor), defense(defense),
          isConsumable(isConsumable), healthRestore(healthRestore) {}
    ...
    bool getIsArmor() const { return isArmor; }
...
public:
    Enemy(const std::string& name, const std::string& description, 
          int health, int damage, int defense, int goldReward)
...
public:
    Player(const std::string& name, Room* startingRoom)
        : name(name), health(100), maxHealth(100), damage(5), defense(0), 
          gold(10), currentRoom(startingRoom), equippedWeapon(nullptr), equippedArmor(nullptr) {}
...
private:
    ...
    void movePlayer(const std::string& direction) {
        ...
        if (nextRoom) {
            ...
            if (nextRoom->getName() == "Ruined Temple" && 
                player->getQuestFlag("ancient_guardian_defeated") &&
                player->findInInventory("Ancient Amulet") != nullptr &&
                player->findInInventory("Guardian Crystal") != nullptr) {
        ...
        if (enemy && enemy->isAlive()) {
            ...
            if (!enemy->isAlive()) {
                ...
                if (enemy->getName() == "Ancient Guardian") {
                    ...
                    std::cout << "Perhaps finding the Ancient Amulet and the Guardian Crystal will reveal more secrets..." << std::endl;
                }
    ...
    void talkTo(const std::string& npcName) {
        ...
        if (currentRoom->getName() == "Small Village" && toLower(npcName) == "villager") {
            ...
            if (!player->getQuestFlag("village_quest_given")) {
                ...
                std::cout << "Villager: \"Legend says that the Ancient Amulet and Guardian Crystal together hold great power.\"" << std::endl;
                std::cout << "Villager: \"If you can defeat the guardian and bring these artifacts together, you might discover a great secret.\"" << std::endl;
                ...
                std::cout << "Villager: \"Now find the Ancient Amulet and Guardian Crystal. Take them back to the temple to reveal its secrets.\"" << std::endl;
            } else {
