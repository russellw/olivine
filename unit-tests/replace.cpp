...
BOOST_AUTO_TEST_CASE(replace_function_ref_test) {
    ...
    BOOST_CHECK_EQUAL(resultCallExpr[0].ref().get<string>(), "new_function");
}
...
BOOST_AUTO_TEST_CASE(replace_preserves_unchanged_elements_test) {
    ...
    BOOST_CHECK_EQUAL(resultFunc.ref().get<string>(), testFunc.ref().get<string>());
